  ...
  return courses.map(course => (
    <Card key={course.id} className="overflow-hidden flex flex-col pb-0">
      ...
      <div
        ...
        style={{ width: `${course.lessonsComplete / course.lessonsCount * 100}%` }}
      />
    </Card>
  ));
}
...
async function getUserCourses(userId: string) {
  'use cache';
  cacheTag(getUserCourseAccessUserTag(userId), getUserLessonCompleteUserTag(userId));
  ...
  }).from(CourseTable).leftJoin(
    UserCourseAccessTable,
    and(eq(UserCourseAccessTable.courseId, CourseTable.id), eq(UserCourseAccessTable.userId, userId))
  ).leftJoin(CourseSectionTable, and(eq(CourseSectionTable.courseId, CourseTable.id), wherePublicCourseSections))
    .leftJoin(LessonTable, and(eq(LessonTable.sectionId, CourseSectionTable.id), wherePublicLessons)).leftJoin(
      UserLessonCompleteTable,
      and(eq(UserLessonCompleteTable.lessonId, LessonTable.id), eq(UserLessonCompleteTable.userId, userId))
    ).orderBy(CourseTable.name).groupBy(CourseTable.id);

  courses.forEach(course => {
    cacheTag(getCourseIdTag(course.id), getCourseSectionCourseTag(course.id), getLessonCourseTag(course.id));
  });
