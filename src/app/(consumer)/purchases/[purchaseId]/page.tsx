...
async function PurchaseReceiptPage({ params }: { params: Promise<{ purchaseId: string }> }) {
  const { purchaseId } = await params;
  return (
    <div className="container my-6">
      <PageHeader title="Purchase History" />
      <Suspense fallback={<LoadingSpinner className="size-36 mx-auto " />}>
        <SuspenseBoundry purchaseId={purchaseId} />
      </Suspense>
    </div>
  );
}
...
async function SuspenseBoundry({ purchaseId }: { purchaseId: string }) {
...
async function getStripeDetails(stripeSessionId: string, pricePaidInCents: number, isRefunded: boolean) {
  const { payment_intent, total_details, amount_total, amount_subtotal } = await stripeServerClient.checkout.sessions
  ...
  const refundAmount = typeof payment_intent !== 'string' && typeof payment_intent?.latest_charge !== 'string'
...
function getReceiptUrl(paymentIntent: Stripe.PaymentIntent | string | null) {
  if (typeof paymentIntent === 'string' || typeof paymentIntent?.latest_charge === 'string') {
    return;
  }
...
function getPricingRows(
  ...
  if (refund) {
    pricingRows.push({ label: 'Refund', amountInDollars: refund / -100 });
  }
