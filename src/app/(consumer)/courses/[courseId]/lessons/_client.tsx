...
function CoursePageClient({ course }: {
  course: {
    id: string;
    courseSections: {
      id: string;
  ...
  const defaultValue = typeof lessonId === 'string'
    ? course.courseSections.find(section => section.lessons.find(lesson => lesson.id === lessonId))
            ...
            {section.lessons.map(lesson => (
              <Button variant="ghost" asChild key={lesson.id}>
                <Link
                  ...
                  className={cn('justify-start', lesson.id === lessonId && 'bg-accent/75 text-accent-foreground')}
                >
                  <VideoIcon />
                  {lesson.name}
