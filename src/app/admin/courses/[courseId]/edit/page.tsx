...
async function EditCoursePage({ params }: { params: Promise<{ courseId: string }> }) {
          ...
          {course.courseSections.map(section => (
            <Card key={section.id}>
              <CardHeader className="flex items-center justify-between gap-4">
                <CardTitle
                  className={cn('flex items-center gap-2', section.status === 'private' && 'text-muted-foreground')}
                >
              ...
              <CardContent>
                <SortableLessonList sections={course.courseSections} lessons={section.lessons} />
              </CardContent>
            </Card>
          ))}
        </TabsContent>
