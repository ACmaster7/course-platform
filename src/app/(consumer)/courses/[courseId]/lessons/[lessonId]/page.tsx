...
async function SuspenseBoundary({ lesson, courseId }: {
  ...
  const canUpdateCompletionStatus = await canUpdateUserLessonCompleteStatus({ userId }, lesson.id);
        ...
        {canView
          ? (
            <YouTubeVideoPlayer
              ...
              onFinishedVideo={!isLessonComplete && canUpdateCompletionStatus
        ...
        <div className="flex justify-between items-start gap-4">
          ...
          <div className="flex gap-2 justify-end">
            ...
            {canUpdateCompletionStatus && (
              <ActionButton
                action={updateLessonCompleteStatus.bind(null, lesson.id, !isLessonComplete)}
                variant="outline"
              >
...
async function ToLessonButton({ children, courseId, lessonFunc, lesson }: {
  ...
  return (
    <Button variant="outline" asChild>
      <Link href={`/courses/${courseId}/lessons/${toLesson.id}`}>{children}</Link>
    </Button>
  );
}
...
async function getPreviousLesson(lesson: {
  ...
  let previousLesson = await db.query.LessonTable.findFirst({
    where: and(lt(LessonTable.order, lesson.order), eq(LessonTable.sectionId, lesson.sectionId), wherePublicLessons),
  ...
  if (previousLesson == null) {
    ...
    previousLesson = await db.query.LessonTable.findFirst({
      where: and(eq(LessonTable.sectionId, previousSection.id), wherePublicLessons),
      orderBy: desc(LessonTable.order),
      columns: { id: true },
    });
  }
...
async function getNextLesson(lesson: {
  ...
  let nextLesson = await db.query.LessonTable.findFirst({
    where: and(gt(LessonTable.order, lesson.order), eq(LessonTable.sectionId, lesson.sectionId), wherePublicLessons),
...
async function getIsLessonComplete({ userId, lessonId }: {
  userId: string;
  lessonId: string;
}) {
  'use cache';
