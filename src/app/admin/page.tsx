...
async function AdminPage() {
  const { averageNetPurchasesPerCustomer, netPurchases, netSales, refundedPurchases, totalRefunds } =
  ...
  return (
    <div className="container my-6">
      <div className=" grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        ...
        <StatCard title="Students">{formatNumber(await getTotalStudents())}</StatCard>
        <StatCard title="Products">{formatNumber(await getTotalProducts())}</StatCard>
        <StatCard title="Courses">{formatNumber(await getTotalCourses())}</StatCard>
        <StatCard title="CourseSections">{formatNumber(await getTotalCourseSections())}</StatCard>
        <StatCard title="Lessons">{formatNumber(await getTotalLessons())}</StatCard>
      </div>
    </div>
  );
}
...
async function getPurchaseDetails() {
  ...
  const data = await db.select({
    totalSales: sql<number>`COALESCE(${sum(PurchaseTable.pricePaidInCents)}, 0)`.mapWith(Number),
  ...
  }).from(PurchaseTable).groupBy(table => table.isRefund);
  ...
  const averageNetPurchasesPerCustomer = salesData?.totalUsers != null && salesData.totalUsers > 0
...
async function getTotalStudents() {
  ...
  const [data] = await db.select({ totalStudents: countDistinct(UserCourseAccessTable.userId) }).from(
    UserCourseAccessTable
  );
...
async function getTotalCourseSections() {
  ...
  const [data] = await db.select({ totalCourseSections: count(CourseSectionTable.id) }).from(CourseSectionTable);
