...
async function ProductPage({ params }: { params: Promise<{ productId: string }> }) {
  ...
  return (
    <div className="container my-6">
      <div className="flex gap-16 items-center justify-between flex-wrap">
        ...
        <div className="relative aspect-video min-w-xs max-w-lg flex-grow">
              ...
              <Accordion type="multiple">
                {course.courseSections.map(section => (
                  <AccordionItem key={section.id} value={section.id}>
                    ...
                    <AccordionContent className="flex flex-col gap2">
...
async function PurchaseButton({ productId }: { productId: string }) {
  ...
  const alreadyOwnsProduct = userId != null && await userOwnsProduct({ userId, productId });

  if (alreadyOwnsProduct) return <p>You already own this product</p>;

  return (
    <Button className="text-xl h-auto py-4 px-8 rouded-lg" asChild>
...
async function getPublicProduct(id: string) {
  ...
  cacheTag(...product.courseProducts.flatMap(cp => [
