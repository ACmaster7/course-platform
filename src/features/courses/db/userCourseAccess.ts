...
export async function addUserCourseAccess(
  ...
  const accesses = await trx.insert(UserCourseAccessTable).values(courseIds.map(courseId => ({ userId, courseId })))
...
export async function revokeUserCourseAccess(
  ...
  const validCourseId = validPurchases.flatMap(p => p.product.courseProducts.map(cp => cp.courseId));

  const removeCourseIds = refundPurchase.courseProducts.flatMap(cp => cp.courseId).filter(courseId =>
    !validCourseId.includes(courseId)
  );

  const revokedAccesses = await trx.delete(UserCourseAccessTable).where(
    and(eq(UserCourseAccessTable.userId, userId), inArray(UserCourseAccessTable.courseId, removeCourseIds))
  ).returning();
